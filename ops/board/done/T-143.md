...
landed: 92c83ddb1974d1cc5d9e65f2fa4c6877cf3dc9d6
---
## Why
...
- Git Bash converts a POSIX `HOME`/`USERPROFILE` value to a Windows path when it execs native python, so `mktemp -d` and the bash-side asserts land on the same real directory — the golden needs no repo-relative scratch dir and leaves no trace.
- The two `✅ … armed:` lines use the contract's pinned spacing (value at column 22); the block above them sits at 23/24, so they read one space narrow. Contract verbatim beat alignment.
- `outputStyle` had to leave bootstrap.py entirely — even inside a comment explaining why it is never written; the verify grep counts occurrences, not assignments. The comment now names what the golden actually asserts: no `~/.claude/settings.json` at all.
