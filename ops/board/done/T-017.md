...
landed: bea57bb71c959a3653563da63a8c958b602a6757
---
## Why
...
- history --tasks finds the oldest wave by awk fixed-string prefix match on `Sprint <n> — ` over first-parent %H %s (subject at col 42); empty match falls back to the tag itself, so hand-tagged sprints keep the v1 range.
- second-seal drill sits after the events/metrics assertions (read-only span, state still parked on the sealed merge) so `metrics done total: 1` stays pinned; a wave-2 done there would flip it to 2.
- reused-number die path drilled with a dangling parentless commit (git commit-tree) standing in for foreign history; asserts message + zero mutation.
- wave-2 recipe on the same integrate branch needs a catch-up first (`git merge --ff-only <base>`) or land can't see the review/ board state — documented in INTEGRATOR.md §4 and MANUAL §Seal.
