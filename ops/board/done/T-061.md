...
map_delta:
landed: 6dc7ca97c3314ed4f1fd1e8a7f0f279b336d1561
---
## Why
...
- The porcelain check needs `-uall`: default `-unormal` collapses a not-yet-tracked reports/ dir to
  ONE `?? docs/sprints/` line that can never equal a file path, so a first render reads as foreign dirt.
- The drill's first-parent assertion is now pinned to the seal's exact subject (`grep -qx`): v2's
  `docs(sprint-N): report refresh` IS a base first-parent commit by design, the seal's still is not.
