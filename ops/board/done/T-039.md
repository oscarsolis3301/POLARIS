...
map_delta:
landed: d958aee1697048f60f2afbeb745e01f6fc6049b6
---
## Why
...
- contract header says 33 core fns; its named list counts 34 — moved the named list verbatim (census: 34/34 in core.sh, 0 left in entry)
- verbatim proof: moved non-blank lines diff-identical to HEAD's ranges; core.sh 298 lines, entry 3541, grand total 3839 (budget [3750,3985])
- update/uninstall re-exec guards copied ONLY the entry script to mktemp — under the pinned loader the copy refuses at startup (caught by drill uninstall); both copies now carry lib/ beside them (`cp -R "${SELF%/*}/lib" "$tmp/lib"`) — the one non-relocation edit, forced by the loader + install-parity example ("the refreshed CLI boots"); CI's POLARIS_UPDATE_REEXEC grep still matches
