...
landed: f4d4013f9d9f76ef6219c79e0eaa7f21cdb6b6c8
---
## Why
...
- board_ref_commit rebuilds the secondary index from disk each commit (read-tree --empty + update-index --add) — deletions from a column mv need no remove call and the branch tree always mirrors disk; seeding read-tree from prev-tip cannot express deletions with --add alone.
- update-ref compare-and-swap is the contention retry: empty old-value asserts must-not-exist, so orphan creation and every advance are race-safe per attempt.
- Windows autocrlf makes update-index print per-file LF/CRLF advice — silenced with 2>/dev/null exactly like the old git-add path; failures still die upstream.
- done gained a fail-closed gate: non-empty map_delta requires $BASE checked out in the primary, checked before any mutation.
- selftest: board-only base commits dropped (empty once the set is gitignored); T-W carries the map_delta fixture; sync-race drill fakes a foreign polaris/board tip via a peer clone + plumbing push.
