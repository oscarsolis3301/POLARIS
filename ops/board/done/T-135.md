---
id: T-135
title: "The stale-tests gate — a mapped surface can't hand off without its tests moving too"
type: feature
scope: ownership
plan: spend-less
points: 5
value: 8
time_criticality: 8
risk_opportunity: 8
wsjf: 4.8
risk: normal
model:
status: done
owner: Owner@DESKTOP
branch: feat/T-135
files_owned:
  - kit/ops/lib/ownership.sh
  - kit/ops/lib/builder.sh
approved:
context_files:
  - ops/contracts/test-surfaces.md
  - ops/contracts/ask-approval.md
  - ops/tests/startup-budget.cmd
  - kit/ops/lib/core.sh
depends_on: [T-134]
contract: ops/contracts/test-surfaces.md
verify:
  - bash -n kit/ops/lib/ownership.sh && bash -n kit/ops/lib/builder.sh
  - test "$(grep -c '^check_freshness()' kit/ops/lib/ownership.sh)" = 1
  - grep -q 'check_freshness "\$ref" "\$id"' kit/ops/lib/ownership.sh
  - sed -n '/^check_freshness()/,/^}/p' kit/ops/lib/ownership.sh | grep -v '^[[:space:]]*#' | grep -c '| owned_match\|| match_one' | grep -qx 0
  - sed -n '/^cmd_guard()/,/^}/p;/^cmd_rules_check()/,/^}/p' kit/ops/lib/ownership.sh | grep -c 'check_freshness\|check_rules' | grep -qx 0
  - grep -q 'SURFACES stale:' kit/ops/lib/ownership.sh && grep -q 'SURFACES violation' kit/ops/lib/ownership.sh
  - grep -q 'no ops/SURFACES.tsv row' kit/ops/lib/builder.sh
  - grep -q 'pack_section "SURFACES' kit/ops/lib/builder.sh
  - bash kit/ops/polaris doctor --fast | tail -1 | grep -q 'fast tier passed'
map_delta: "ownership.sh gains check_freshness (the stale-tests gate over ops/SURFACES.tsv), reached through check_rules at verify/handoff/audit/land, never from the write guard; approve also clears a SURFACES row; pack prints the SURFACES section"
landed: 481590bd60279fbf68c1a3468a9f5eec7bd7b5a7
---
## Why
Nothing in POLARIS notices when a feature changes and its tests do not. This task adds the second
reader of `ops/SURFACES.tsv`: `check_freshness`, a scanner beside `check_rules` with the same shape
— every changed path of the branch, matched against each row's `surface` and `tests` globs. A
change with real added lines under a mapped surface and no change under its tests fails `verify`,
naming the row and the tests to update. It is reached THROUGH `check_rules`, so verify, handoff,
audit and land all get it with zero new call sites — and the write-time guard never does (it sees
one path at a time and could only ever deny the first edit of every task).

The hard part is false positives, and the answer is never "delete the row": docs-only and
comment-only diffs do not count; a change that touches the tests is fine by construction; a
genuine exception is a human's recorded `polaris approve <ID> <surface> -m "why"` — the same path
that lifts `ask` rules, so `approve` learns that a SURFACES row is also something worth approving.
`pack` shows a Builder up front which tests it is expected to move. Contract § 5 pins the algorithm,
the deny lines and the exemption; T-138 proves them in the drill, so make every message match.

## Acceptance criteria (binary — each becomes a test; put the runnable ones in verify:)
- [ ] `check_freshness <ref> [<ID>]` per contract § 5: inert without rows; S/T sets computed as pinned (added non-blank, non-`#`, non-`//` lines under surface; any change under tests); violation ⇔ S non-empty and T empty; deny lines and trailer byte-exact; `surfaces clean: <k> row(s) checked` on a clean pass with rows present
- [ ] exemption: every path in S covered by an `approved:` entry ⇒ pass + `⚠ SURFACES exception used — …` note per path; no ID or `-` ⇒ no approvals
- [ ] `check_rules` runs the freshness pass in ALL cases (its early returns become skips) with two flags — the `RULES violation` trailer never prints for a freshness-only failure; with no rules and no rows the output is byte-identical to today
- [ ] `cmd_guard` and `cmd_rules_check` still never call `check_rules`/`check_freshness` (D4); `per-rule-match-pipes 0` holds (args, never a pipe, inside `check_freshness`)
- [ ] `cmd_approve`'s no-op precondition widened to `ask_rule_matches || surface_rows_for … | grep -q .` with the pinned die text; the feat/* refusal and the recorded entry unchanged (the `rules` drill's ask assertions stay green)
- [ ] `cmd_pack` prints the pinned SURFACES section only when a covering row or a `surface:` item exists; `pack-visual` golden and every existing pack output byte-identical
- [ ] proven in a scratch repo (record the transcript lines in Notes): src-only change ⇒ verify rc 1 with `SURFACES stale:`; tests change added ⇒ rc 0; comment-only ⇒ rc 0; approval ⇒ rc 0 with the exception note
- [ ] no new top-level fn beyond `check_freshness` (T-136 writes its api-kit row); no heading; bash 3.2 clean

## Notes
<!-- Builders append one-line discoveries here; Integrator pastes failures here (path:line only). -->
- Scratch paths under `scratchpad/T-135/…` only; long suites via `bg run` + chunked `bg wait`, never a foreground call over 600s. Trust the rc, never the refusal text (T-089).
- proof (scratch repo, kit from feat/T-135): src-only change ⇒ verify rc 1 · `⛔ SURFACES stale: src/sf/a.txt changed under 'src/sf/' but nothing under 'tests/sf/' did — the sf surface [T-0]` + `… polaris approve T-9 src/sf/ -m "why"`; handoff rc 1, task still in active/; two S paths ⇒ `src/sf/a.txt (+1 more)`
- proof: tests/sf/ change added ⇒ rc 0 `✅ surfaces clean: 1 row(s) checked`; comment-only (`#` and `//`) added lines ⇒ rc 0; an unmapped src/other/ change ⇒ rc 0 (D3); no rules + no rows ⇒ verify output byte-identical to before
- proof: approve T-9 src/sf/ from the primary ⇒ verify rc 0 with `⚠ SURFACES exception used — src/sf/a.txt — stale-tests gate on 'src/sf/' cleared by approved: src/sf/ — Owner@DESKTOP, 2026-09-14: drill`; approve on feat/* still refuses; an ungated scope ⇒ the pinned `nothing gates` die, nothing written
- proof: pack prints the SURFACES section only with an overlapping row or a surface: item (a malformed item ⇒ `⚠ malformed: needs 'tests: <glob>'`); no rows ⇒ absent, so pack-visual stays byte-identical
- assumed: pack's row list uses pat_overlap (both directions, deduplicated) so a row nested under an owned directory shows too — a superset of "covering row", informational only (default-safe)
- check_freshness diffs a path once, and only when some row maps it (surface hit, tests miss); api-kit gains exactly the one pinned row `kit/ops/lib/ownership.sh	fn	check_freshness` (T-136 writes it)
