...
map_delta:
landed: 7c9f0977d3bf48824c936d773c17ee2f509ac6c2
---
## Why this exists
...
- Chained behind T-005 (same hotspot file). Kit/ops/polaris chain updated 2026-07-18: T-004 → T-013 → T-017 → T-005 → T-006.
- Kept the `command -v unzip` guard per acceptance criterion 4 (no new dependency) — did not switch to kit_zip_version's python path despite the earlier note's suggestion; that would have contradicted "existing guard pattern stays".
- `cmd_update` already always fetches the tarball fresh (no cache read) and clears `.polaris/update-cache` on success — only `cmd_version` needed the throttle-bypass; `update_check_maybe` now takes an optional `force` arg.
- Added selftest drills (stale-zip gate + version-freshness bypass) using fake `curl`/`unzip` shims on PATH — Git Bash ships neither `zip` nor a mockable network, so this keeps the drill deterministic and portable instead of building a real fixture zip.
