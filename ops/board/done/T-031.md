---
id: T-031
title: land --express — one-pass small-change landing + slow-suite hint
type: feature
scope: cli
plan: fast-lane
points: 5
value: 8
time_criticality: 8
risk_opportunity: 3
wsjf: 3.8
risk: normal
status: done
owner: Owner@DESKTOP
branch: feat/T-031
files_owned:
  - kit/ops/polaris
context_files:
  - ops/contracts/verification-tiering.md
  - ops/CONVENTIONS.md
depends_on: [T-030]
contract: ops/contracts/express-lane.md
verify:
  - grep -q -- '--express' kit/ops/polaris
  - grep -q 'express lands exactly one task' kit/ops/polaris
  - grep -q 'risk: high never rides the express lane' kit/ops/polaris
  - grep -q 'express needs publish: direct' kit/ops/polaris
  - grep -q 'last-suite-seconds' kit/ops/polaris
  - grep -q 'suite last took' kit/ops/polaris
  - bash kit/ops/polaris doctor --selftest
map_delta: land gains --express <ID> — audit+land+ONE full suite+seal+run-verify+done in one pass, refusals per express-lane contract; qa stamps suite duration, land hints when a paranoid suite ran >2min (5.15.0-unreleased)
landed: d415c1ef7de37b1d67bede3f03712a0091afa19d
---
## Why
A 1-file 2-point request pays the same session ceremony as a 25-point sprint — that is why small
asks take hours. `land --express <ID>` collapses the integrator's long path (integrate branch →
audit+land → full suite → seal → run-verify → done → branch cleanup) into one command for the
single-task case, refusing loudly whenever the case is not single/safe (four pinned refusals in the
contract). Every gate still runs — express collapses sessions, never checks. Same task, second seam:
`qa` records how long the suite took (.polaris/last-suite-seconds) and `land` prints a one-line hint
when a paranoid-mode suite exceeds 2 minutes, making INTEGRATOR.md's batch-first rule mechanical.

## Acceptance criteria (binary — each becomes a test; put the runnable ones in verify:)
- [ ] `land --express <ID>` runs the contract's 5 steps and exits green on the happy path: task in done/ with a landed: stamp, sprint tag set/moved, integrate branch deleted, tree clean
- [ ] all four refusals die BEFORE any mutation, each printing its pinned fragment
- [ ] a red suite mid-express unwinds the land (reset --hard HEAD~1), kicks the task back with the failing tail, and dies
- [ ] `land <ID>` without --express is byte-identical to today
- [ ] `qa` writes .polaris/last-suite-seconds ("<seconds> <epoch>") only when ≥1 suite command ran
- [ ] `land` prints the `suite last took` hint ONLY when integration: paranoid AND stamp >120s; exit status never changes
- [ ] happy-path + 4 refusal + hint/silent drills ride selftest(); existing drills untouched
- [ ] `polaris help` shows the --express form

## Notes
- bash >= 3.2: NO `case` inside `$(...)`; split `local` declarations when one references another (T-029).
- Unknown `express:` value behaves as OFF (fail to the full ceremony), warned once — see contract.
- Express drill must restore publish: direct / integration defaults it changes, or later drills break.
- refusal order: 4 pinned refusals precede primary/base/clean-tree context checks, so refusals fire from any CWD/tree state
- happy-path drill reuses the red drill leftover integrate/<today>, covering step 1 reuse+ff free
- hint reads cfg integration with default batch (kit default per INIT.md) — unset key never fires
