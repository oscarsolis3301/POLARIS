---
id: T-114
title: "wt_remove must never delete the worktree holding the running script or your cwd — beat age must not be able to override it"
type: bug
scope: workspace
epic: cant-eat-itself
plan: cant-eat-itself
points: 2
value: 13
time_criticality: 8
risk_opportunity: 3
wsjf: 12.0
risk: normal
status: done
owner: Owner@DESKTOP
branch: feat/T-114
files_owned:
  - kit/ops/lib/workspace.sh
approved:
context_files:
  - ops/contracts/worktree-liveness.md
  - kit/ops/lib/builder.sh
  - kit/ops/lib/integrate.sh
  - kit/ops/lib/observe.sh
depends_on:
  - T-092
  - T-099
contract: ops/contracts/worktree-liveness.md
verify:
  - bash -n kit/ops/lib/workspace.sh
  - grep -q 'holds the running script' kit/ops/lib/workspace.sh
  - grep -q 'is your current directory' kit/ops/lib/workspace.sh
  - test "$(grep -n 'holds the running script' kit/ops/lib/workspace.sh | head -1 | cut -d':' -f1)" -lt "$(grep -n 'beat_age "$id"' kit/ops/lib/workspace.sh | head -1 | cut -d':' -f1)"
  - test "$(grep -cE '^[a-z_]+\(\)' kit/ops/lib/workspace.sh)" = 14
  - grep -q 'worktree LEFT: ' kit/ops/lib/workspace.sh
map_delta:
landed: 41ba55460f6de5687d17c82eafe158dd2a7f383b
---
## Why
**This one deleted work.** On 2026-09-02 the T-104 lane's own self-land removed the worktree it was
running from; `$SELF` vanished mid-execution and the seal fan-out died, stranding six landed tasks in
`review/` with orphaned worktrees. It is the ARC-428 class this whole sprint exists to end, and it is
live in `main` right now.

The chain, all of it landed code, every step behaving as designed:

1. A builder runs `bash ops/polaris handoff` FROM its worktree, so `$SELF` is
   `.polaris/wt/<ID>/ops/polaris` — **the running script lives inside the worktree.**
2. `cmd_handoff` beats that worktree once (`kit/ops/lib/builder.sh:213`).
3. The landing tail is long: lease wait → `land` → `seal` → suite → fan-out. **Nothing re-beats
   during it.** T-104's tail ran well past `wt_live_minutes` (default 15).
4. The fan-out runs `( cd "$PRIMARY" && "$SELF" done "$tid" )` for every landed review task,
   including the lane's own id. `done <own-ID>` runs from the primary, so `wt_remove`'s existing
   `own` test (`case "$PWD" in */.polaris/wt/<ID>...`) does not fire.
5. `beat_live` says idle → `refuse=0` → `git worktree remove` → rc 0. The worktree, and the script
   executing out of it, are gone. Every remaining `done` in the loop then fails.

The contract's § decision table literally asserted this could not happen — "the handoff just beat, so
`done` on the builder's own worktree hits clean+LIVE ⇒ LEFT … Nothing ever removes the worktree a
session is standing in." That reasoning is only true while the beat stays fresh, and the tail outlives
it. `ops/contracts/worktree-liveness.md` § v1.2 now pins the correction; build to it.

**The change.** In `wt_remove`, add a refusal evaluated **BEFORE** the liveness/dirty decision — before
`beat_age`/`beat_live` are consulted at all, so no beat age, no caller and no `--fix` can override it.
Hard refusal: **LEFT, rc 1**, nothing touched, for every caller (`done`, `release`, `sweep`) and every
cell of the decision table. Two conditions, either one refuses:

1. `$SELF` — the running script, resolved to an absolute real path — is inside the target worktree.
2. `$PWD` is the target worktree or inside it.

Resolve both sides to absolute paths and normalize separators before comparing: on Windows the
worktree path can arrive with mixed `\` and `/`. `next_route` in `kit/ops/lib/handover.sh` already
does this for bg-job cwds (`tr 'A-Z\' 'a-z/'`) — copy that shape rather than inventing one.

Pinned notes, byte-exact (the `worktree LEFT: ` prefix is unchanged so existing drill greps still
match; the `$SELF` note wins when both conditions hold):

```
worktree LEFT: .polaris/wt/<ID> holds the running script — never remove the ground you are standing on; a later sweep --fix finishes it once the session is gone
worktree LEFT: .polaris/wt/<ID> is your current directory — cd out and run: bash ops/polaris sweep --fix
```

**Inline the guard — NO new top-level function.** `workspace.sh` has exactly 14 top-level functions
and the api-kit golden pins every name; adding one reds a golden this task does not own. (Same call
the shared-checkout v1.1 amendment made for the mutex owner guard.)

This is one of two guards. T-115 stops the observed trigger by making the fan-out skip its own task;
this task makes the whole class impossible — a hand-run `done`, a `sweep --fix` from inside a
worktree, any future caller. Both are required before T-108 ships 6.2.0. They own different files and
can run in parallel.

## Acceptance criteria (binary — each becomes a test; put the runnable ones in verify:)
- [ ] **Regression proof, both directions.** A fixture worktree whose beat is deliberately backdated
      PAST `wt_live_minutes` (`echo 1 > "$(git rev-parse --git-common-dir)/worktrees/<ID>/polaris-beat"`),
      with the CLI under test living inside that worktree, is still LEFT by `done` AND by `sweep --fix`:
      rc 1, the pinned `holds the running script` note, and **the worktree directory still exists
      afterwards**. Assert the directory's survival and the rc — never the note alone.
- [ ] **Sabotage:** with the guard removed the same fixture DELETES the worktree and the assertion
      reds; with it restored, green. Record both outcomes in Notes.
- [ ] Standing inside the target worktree, `sweep --fix` refuses with the `is your current directory` note
- [ ] The guard runs before any beat/liveness call (verify 4)
- [ ] A worktree that holds neither `$SELF` nor `$PWD` behaves EXACTLY as the v1 decision table says —
      no row changes; re-read the table and spot-check clean+idle removal still returns rc 0
- [ ] No new top-level function; `bash -n` clean (verify 5, 1)

## Notes
<!-- Builders append one-line discoveries here; Integrator pastes failures here (path:line only). -->
- Filed by the planner 2026-09-02 from a live failure. Evidence at filing time: primary on `main`,
  six orphaned worktrees (T-105, T-106, T-107, T-111, T-112, T-113) with beats 14-79 minutes old, and
  those same six tasks stranded in `review/`.
- `verify:` entries 2, 3 and 4 are RED on today's `main` and go green with the guard; entries 1, 5 and 6
  are guards that must stay green. The behavioural regression proof is an acceptance criterion, not a
  `verify:` entry, because `verify:` runs 2-3x per task and the wave gate already runs the suite once.
- Do NOT weaken `beat_live`, `beat_age` or `beat_touch` to reach this. An EMPTY beat file is CORRECT
  (the zero-fork hooks touch mtime only; the mtime fallback in `beat_age` is the designed path) —
  contract § v1.2 says so explicitly so nobody "repairs" it later.
- Regression proof, guard ON (throwaway repo, task landed+sealed, beat backdated to epoch 1 = 29.8M
  minutes idle, CLI invoked from the primary through `<wt>/ops/polaris`): `done` LEFT it with the
  pinned `holds the running script` note and the directory alive; `sweep --fix` the same; the
  primitive itself returned rc 1; the `$PWD` arm (cwd inside, `$SELF` outside) printed its own pinned
  note, rc 1, directory alive.
- Regression proof, guard OFF (same fixture, guard block excised): `done` printed `worktree removed`
  and DELETED the worktree, after which the next call died with
  `bash: <wt>/ops/polaris: No such file or directory` — the exact symptom that stranded six tasks.
- The v1 table is unmoved: in the same guard-ON sandbox a worktree holding neither `$SELF` nor `$PWD`
  (idle + clean, `done/`) is still removed, rc 0. Drills `wtreap`, `selfland`, `park` + spine green.
- Past the guard `own` is always 0 (its `$PWD` arm subsumes it). The two `own` reads are left verbatim
  so no other cell of the table moves — the own-lane `release` rows are now unreachable by design,
  which is exactly what contract § v1.2's amending sentence says.
- The `is your current directory` arm is NOT reachable through a short CLI run: the entry preamble
  beats the worktree containing `$PWD` at process start, so `sweep` reports it LIVE and never calls
  `wt_remove`. It exists for the long-running process whose tail outran `wt_live_minutes` — proved by
  direct call, which is where it lives.
- Windows needed BOTH comparison shapes: git hands back `C:/…` while `$PWD`/`$SELF` are `/c/…`, so the
  canonical `cd`+`pwd` form is folded with `tr 'A-Z\' 'a-z/'` and the `*/.polaris/wt/<ID>` suffix test
  (the pre-v1.2 own-lane shape) is kept alongside it.
