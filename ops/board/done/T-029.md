...
map_delta:
landed: f2bbe3a1e4e5c3a1285385e46a990da82304a0ed
---
## Why
...
## Notes
- Fix: split `local n="$1"` onto its own line before the `local tag=…/prev=…` decl (resolve_sprint_ids ~:1292). bash expands every word of a `local` line before assigning, so the combined decl read `$n` from the caller — "" under `cmd_report --all` (loops with `m`, own `n=""`).
- report_one/seal_report_commit/cmd_report --sprint were byte-identical before and after: each caller holds `local n="$1"` equal to the arg, so the buggy combined decl happened to read the right value; only `--all`'s empty caller-n was wrong.
- Drill: inserted a Rule-2-blind sub-drill after `done T-1` (selftest ~:1752). Save main, `git commit --amend` the seal merge to drop the `[T-1]` bullet (keep the `Sprint 1 — …` subject) + move tag, `report --all`, assert `## T-1` present and NO `## (unsealed)`, then reset tag+main so the rollback/second-seal drills below stay byte-identical.
- Verified RED: reverting only the split to the combined decl fails the new drill with `REPORT RULE2 UNSEALED FAIL`; full selftest `SELFTEST-PASS` on the fixed code; build (pack.py) green.
