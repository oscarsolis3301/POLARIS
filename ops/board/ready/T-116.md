...
contract:
verify:
  - 'ver="$(sed -n "s/^version: *//p" kit/ops/VERSION | head -1)"; bad=$(grep -rhoE "POLARIS [0-9]+\.[0-9]+\.[0-9]+" README.md kit/.claude/skills/ kit/ops/*.md 2>/dev/null | sed "s/^POLARIS //" | sort -u | grep -vFx "$ver" || true); [ -z "$bad" ] && echo "sweep clean at $ver" || (echo "STILL BAD: $bad"; exit 1)'
