...
- v2.3 2026-07-20 (per T-038 Notes): per-stream clarification of v2.2(b) — git-for-Windows 2.53 emits the two squash notes on DIFFERENT streams ("Squash commit -- not updating HEAD" → stdout; "Automatic merge went well; stopped before committing as requested" → STDERR on a divergent merge). land captures BOTH streams of the squash-merge command: dropped on success, captured stderr re-emitted on conflict/failure before kickback so real failures stay visible. Supersedes the literal "stdout only"; documents shipped T-038 behavior — no code change.
