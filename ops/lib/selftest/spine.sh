...
selftest() { # end-to-end mechanics drill in a throwaway repo — run once per new machine.
  ...
  # Every label a drill_on gate below uses — the contract's minimum set. An --only element matching
  # NONE of these dies HERE, before the throwaway repo is even created. Order is the DECLARED order
  # (it only feeds the round-robin shard split and the die text); the three sprint-12 labels are
  # appended as a block, while their gates sit in the run order worktree-liveness.md / keep-awake.md
  # / role-handover.md ask for: awake after checkoutguard, handover after readyoverlap, wtreap after
  # selfland; surfaces (T-138, test-surfaces.md § 10) is appended after them and its gate sits right after
  # rules; skills (T-159, self-skills.md § 9) is appended LAST — so `--only surfaces` globs are unmoved —
  # and its gate sits right after qa, on its own throwaway board.
  local SELFTEST_LABELS='fmlist tcm report metrics brain rules drift hardening qa finish claudemd remote syncrace notify grant upgrade adopt pr-publish express hint brief newcmds park claimguard busyint pushdegrade route bg checkoutguard readyoverlap selfland wtreap awake handover autoupdate surfaces skills'
  ...
  local T; T="$(mktemp -d)"
  export POLARIS_AWAKE_HOME="$T/awake-home"   # keep-awake.md: awake_ensure fires from claim/status/doctor/handoff/bg run, so EVERY drill would otherwise register this throwaway repo in the owner's real ~/.claude/polaris/awake — the registry follows the fixture and dies with it
  ...
  ( set -e; cd "$T"
    ...
    # THE LANE WALKED AWAY (worktree-liveness.md, the `done` row + `echo 1 > <beat>` fake-idle form).
    # cmd_handoff beats its own worktree on purpose, so a done seconds later sees clean+LIVE, LEAVES
    # the worktree and KEEPS feat/T-1 — correct for a real session still typing, but here it left the
    # fixture with cruft branches and drill_qa's healthy repo went red on drift. A drill that wants
    # done to reap must say the session is gone, and the beat is the only thing that says it.
    echo 1 2>/dev/null > "$(git rev-parse --git-common-dir)/worktrees/T-1/polaris-beat" || true
    ...
    echo 1 2>/dev/null > "$(git rev-parse --git-common-dir)/worktrees/T-W/polaris-beat" || true   # the lane walked away — see the T-1 done above
    ...
    echo 1 2>/dev/null > "$(git rev-parse --git-common-dir)/worktrees/T-Z/polaris-beat" || true   # the lane walked away — see the T-1 done above
    ...
    if drill_on surfaces; then
    drill_surfaces
    fi   # drill_on surfaces
    ...
    if drill_on skills; then
    drill_skills
    fi   # drill_on skills
    # finish goes HERE, right after qa: it asserts whole-board quiescence, so it must sit where the
    # board is provably quiet — both spine tasks drained to done/ through real seals, both locks
    # dropped, clean tree on the base branch — and drill_qa leaves CONVENTIONS.md absent, so cfg
    # returns pure defaults. Later positions (after express) leave a task in done/ and a mutated
    # CONVENTIONS.md behind.
    if drill_on finish; then
    drill_finish
    fi   # drill_on finish
    if drill_on claudemd; then
    drill_claudemd
    fi   # drill_on claudemd
...
🤖 Generated with [Claude Code](https://claude.com/claude-code)'
    ...
    # adopt rides beside upgrade deliberately: they are the two halves of "an installed repo meets a
    # newer kit" — upgrade migrates the board, adopt discloses the keys the new kit reads. It is
    # hermetic on its own (T-046), so any --only/--parallel partition meets the same spine.
    if drill_on adopt; then
    drill_adopt
    fi   # drill_on adopt
    ...
    # check). Deliberately last among the BOARD drills: park/busyint land + seal real waves and
    ...
    # ==== T-071 routing + background-job drills (ops/contracts/model-routing.md ·
    # ops/contracts/bg-jobs.md). They follow the board drills deliberately, for the mirror of that
    # reason: `route` writes nothing at all, but drill_bg drives two REPO-WIDE commands — `finish`
    # and `sweep --fix` — and the safest place for a command that tidies locks, branches and job
    # dirs is where nothing downstream can inherit what it tidied. Both are hermetic on their own
    # (T-046): drill_route restores CONVENTIONS.md byte-exactly, drill_bg removes the whole
    # .polaris/bg registry, so any --only/--parallel partition meets the same spine.
    if drill_on route; then
    drill_route
    fi   # drill_on route
    if drill_on bg; then
    drill_bg
    fi   # drill_on bg
    # ==== end T-071 routing + background-job drills ====
    # ==== T-089 enforced-isolation drills (ops/contracts/shared-checkout.md v2 §6). Last among
    # the drills deliberately: checkoutguard's no-lock case needs a lock-free repo (it asserts the
    # precondition and names any leak), and selfland lands + seals real waves — nothing downstream
    # can inherit their state. Each is hermetic on its own (T-046), so any --only/--parallel
    # partition meets the same spine.
    if drill_on checkoutguard; then
    drill_checkoutguard
    fi   # drill_on checkoutguard
    if drill_on awake; then
    drill_awake
    fi   # drill_on awake
    if drill_on readyoverlap; then
    drill_readyoverlap
    fi   # drill_on readyoverlap
    if drill_on handover; then
    drill_handover
    fi   # drill_on handover
    # autoupdate (ops/contracts/auto-update.md § Executable check) sits right after handover: it
    # needs the board provably quiet (every spine task drained through real seals, no lock, no
    # lease) and it installs a real kit into this fixture — hermetic on its own (T-046): the
    # installed tree is committed and reset away, ops/VERSION, CONVENTIONS and src/a.txt restored.
    if drill_on autoupdate; then
    drill_autoupdate
    fi   # drill_on autoupdate
    if drill_on selfland; then
    drill_selfland
    fi   # drill_on selfland
    if drill_on wtreap; then
    drill_wtreap
    fi   # drill_on wtreap
    # ==== end T-089 enforced-isolation drills ====
  ...
  say "selftest passed: race(8→1) · ownership accept/reject · verify cmds · handoff+all-review notice · fm_list scalar/[]/flow/block+messy/depends_on · task-commit-msg format · land squash+trailer · seal merge+tag · history filter · done-on-squash+landed stamp · rollback task+sprint · second-seal tag-move+wave-spanning history+wave-2 done · quiet board (chores→polaris/board · zero base chores · orphan root · docs(map) once · tree=moved set · clean status) · EVENTS union sync race · legacy --no-ff done · done cleanup · events+metrics(+pts buckets) · _match · rules path/content/diff · drift overlap+strict · rename-reject · claim fan-out · glob-overlap · audit reject · why · dep-cycle · qa green/red · finish (pending named + rc 1 + no stamp/hook · drain queue-gates vs plan-caveats · worktree refusal · green verdict + stamp format · done hook fires EXACTLY once, re-run stays rc 0) · CLAUDE.md block version truth (stamp mismatch names both · unstamped · unmarked · SILENT when current) · attribution strip · remote branch cleanup+sweep · notify-gate silence/env/severity · blocked event+why · severity info/gate · doctor knob composition (6.0 flip: unset composes auto/default-safe/auto-reversible · autonomy: standard restores confirm/ask/confirm · explicit beats the dial both ways · an unknown value on a knob OR on the dial falls back to STANDARD — a typo never grants autonomy) · adopt (both refusals · append-only in bytes · every appended line commented · a live value and an existing stub survive · byte-identical no-op re-run · a grown registry reuses the one marker) · grant append+refusals(column/-m/overlap both ways) · upgrade-migration (orphan seed · untrack+ignore · ONE base commit · no-op re-run · quiet after) · primary-anchored claim/resume paths · fresh-clone materialization (doctor+resume) · uninstall board-branch delete local+origin · pr-publish (feat stays local · seal pushes ONLY integrate + title/bullets/URL + notify done, no ref moved · --sync subject gate/tag/branch cleanup both sides · done via rule 1 · direct --sync dies · unknown publish warns) · sprint report (seal commits docs(sprint-N) riding the wave · cmd_report --sprint/current/--all renders + prints, board read-only + idempotent) · brain (9-file build gitignored · INDEX routes all 7 + find · prefs/learned content · board digest names the landed task · stale warn → refresh clears · seal auto-refresh · refresh failure = note, never a red seal) · express (help form · qa suite stamp gated+formatted · 4 refusals pre-mutation: solo-review/risk-high/off+unknown/publish-pr · red suite unwind+kickback tail · happy path: done+landed stamp · tag moved · integrate deleted · clean tree · finish-line note names polaris finish) · slow-suite hint (paranoid+180s fires naming integration: batch · batch silent) · status --brief (one paragraph · Last landed:/Next up: markers · header sprint clause · active ids · no table pipe · plain status unchanged) · metrics In-plain-English summary (first line above the table · silent on empty EVENTS) · park (tracked+untracked parked · byte-identical unpark · a dirty tree at land parks + proceeds + prints the stash) · claimguard (id_ok pre-lock · stray feat ref archived local+origin at claim · overlap auto-block + remedy note) · busyint (busy lease → wait note + queued rc 3 · stale lease stolen · already-landed skip rc 0 · non-ff wave adopted + sealed once) · pushdegrade (stray-feat repair mid-handoff · dead push degrades: board moves + ⚠ Note + push-fail event) · route (tier table incl. risk dominance + non-numeric points · role table + unknown-role fallback · unset-knob SILENCE · three-space note shape per tier · tier-word and literal model: overrides · fleet carries the ready queue's max tier, and no token when unset) · bg (rc contract 0 green/1 red/2 running/3 dead-pid unknown · duplicate-RUNNING refusal + --force replace · tail · .prev rotation archives the previous run · finish names an uncollected job · sweep --fix rotates a >24h job) · checkoutguard (primary git switch → hookSpecificOutput JSON deny on stdout, nothing on stderr · worktree cwd + read-only git + the stash list/show carve-out allowed · primary_gate: live lock + tracked source denies exit-2+stderr · board path + lock-free repo allowed · fallback entry: absolute .polaris/wt/<ID> write allowed by BOTH hooks + the commit lands on feat/<ID>) · readyoverlap (explicit claim dies overlaps ready · auto-pick parks the collider in blocked/ + remedy + claims the next in the same pass · drift reports always, only --strict fails) · selfland (unset knob composes self: handoff lands + seals + dones itself, lease released · risk: high refuses with the pinned line and stays in review/ · landing: integrator restores the classic notice · the self-landed lane keeps its OWN worktree, and an idle sweep --fix reaps it) · wtreap (the caller × dirty × live table end to end: done archives a dirty idle worktree with its bytes and drops the branch · done LEAVES a live one and keeps the branch · release from outside dies naming the beat file, then archives once the beat is gone · resume refuses another session and admits its own · sweep reports LIVE, then --fix removes clean and archives dirty · a hook payload from a worktree git never heard of stays silent) · awake (the four machine hooks write the session file with EMPTY output and never downgrade busy · a busy session presses within 3s · all-idle exits after GRACE and drops the lock · a stale lock is stolen · stop leaves, disabled never presses · re-entrant ensure = ONE daemon · a live bg job alone keeps the machine up · install keeps a foreign entry and replaces ours) · handover (role-handover.md § executable check)· autoupdate (auto-update.md § executable check: quiet board + --auto applies from a file:// channel → ops/VERSION 0.0.2 + update.log + the one ✅ line · a task in active/ → skipped: active: 1, unchanged · auto_update: off → skipped, unchanged · a MAJOR on the channel → the ask line, rc 0, unchanged · --auto never parks kit dirt · explicit update with app-only dirt lands with NO stash + the left-alone note · kit dirt parks + lands · --all over a registry: prefixed apply line · gone · self-hosting — skipped · entry never deleted · fixture restored byte-for-byte) · surfaces (test-surfaces.md § 10: the row written by done from a surface: item + ONE docs(surfaces) commit · stale-tests gate refuses source-only on stderr, handoff refuses and the task stays active, passes once the tests move · comment-only gates nothing · a human's approve is the recorded exception and feat/* still refuses · qa scoped to the mapped cmd + scoped stamp, whole suite + full stamp on an unmapped change, --full never selects, a HEAD stamp skips naming what was proven, a - row runs the test_select: template · a self-covering row: surfaces rc 1, drift finding, --strict rc 1, clean once removed · hermetic) · skills (self-skills.md § 9, on its own throwaway board: gaps folds seven done tasks to ONE candidate · propose --write born hidden with the seven headings + the twin per the probe, a second --write refused · promote refuses a TODO and a shelf crossing writing nothing, passes a written description · claim writes ONE skill-hit · every writer refuses on feat/* writing nothing · prune: young rc 0 → demote at 41 done (--apply flips the flag) → keep while the hit is inside 2W → archive at 81 (dir + twin moved, git ls-files drops both) · restore byte-identical from the archive and from git history, refuses a present dir · hermetic)"
}
