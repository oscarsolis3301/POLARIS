...
**A verify line the builder proved unsatisfiable by construction is amended from the primary** — `bash ops/polaris amend <ID> --verify <n> -m "why" -- <cmd>` — never by hand, never by the builder. That is the sibling of `grant`: `grant` widens what a claimed task may touch, `amend` corrects what it must prove. The usual case is a cross-lane golden owner whose line cannot pass until a sibling lands; the why lands on the task's record, and the command refuses a bare full-suite line, so the wave gate stays the wave gate.
...
**Open the PAIR, not one picture — and read the caption beside it.** Before landing a visual task, open BOTH captures, before and after (`audit` prints a `capture:` line per shot it finds), and read the builder's `--saw` text next to them. One picture only proves a capture happened; before and after side by side is the only view that shows whether anything actually improved, and the `--saw` line is the builder's own claim about that pair, measured against `ops/DESIGN.md`. When the caption and the pictures disagree, that is a kickback — `bash ops/polaris kickback <ID> -m "<what the shots actually show>"` — not a note in your report. A green suite shipped a broken page once.
...
## 6. Close the loop
**`seal` already wrote the burndown row; a lesson is `bash ops/polaris learned -m "…"` (≤3 per wave) — never a hand edit of SPRINT.md.** Both land on the `polaris/board` ref in their own commits, so there is nothing here for you to commit. Run `bash ops/polaris metrics` for the cycle-p50 + kickback numbers — that's what EVOLVE and the Planner calibrate on — and spend your ≤3 bullets on what the Planner should carve differently next time: ownership violations, conflict causes, flaky tests. A wave that taught you nothing gets none.
