...
A third refusal comes from the same gate: `verify` — and `handoff`, `audit` and `land` after it — refuses a change to a mapped surface whose tests did not change: `⛔ SURFACES stale: <path> changed under '<surface>' but nothing under '<tests>' did`. `ops/SURFACES.tsv` maps which tests cover which source paths (`pack` prints the rows that cover what you own). The fix is the tests, never the map: that file is written only by `polaris done` from a task's `surface:` list and is RULES-guarded, so a row edit is the same offense as a RULES edit. A genuine exception is a human's recorded decision — `polaris approve <ID> <surface> -m "why"`, which you cannot run — so hand back for it. A wrong or missing row → one line in `ops/board/backlog/IDEAS.md`, or a `surface:` proposal in your task's Notes for the Planner. A changed path with no row is simply ungated (unmapped is silent by design) — not a licence to map it yourself.
...
## 4. Test
Write tests covering EVERY acceptance checkbox. Then run the commands from `ops/CONVENTIONS.md`: **`test_fast:` when it is set** — plus `lint:` and `typecheck:`. All green or you stay in `active/`. **When `test_fast:` is unset there is NO fallback to `test:`.** The full `test:` is the WAVE gate and is paid ONCE — at the wave gate, in the Integrator's `qa`, at `finish`, and in CI — and `finish` skips it outright when `.polaris/suite-stamp` already names HEAD (an express land writes that stamp since 6.3.0). Running the full suite per handoff is how a one-line change came to pay it twice, and a suite over the harness's tool timeout returns NOTHING and gets re-run, which is worse than useless. So: fast tier here, full suite once per wave — you are not skipping a gate. (Working on POLARIS itself? Its own seconds-long check is `bash ops/polaris doctor --fast`.)
...
**4b. See your work — before AND after.** If `pack` printed a SEE YOUR WORK section and your diff touches a `visual:` path: run the printed `shot:` line BEFORE you edit and again after, both into the `shotdir` `pack` named, then READ both — `read: ops/DESIGN.md` is the bar this screen must clear. The capture lock queues you; never fight it, never build your own capture tool, never force opacity to fake a paint — a blank image, or one nobody opened, proves nothing. Hand off with `--saw "<what the after shot shows, and how it measures against the bar — PASS / WEAK / FAIL>"`; a brand-new screen with nothing to photograph first adds `--no-before "<why>"`. `handoff` refuses without two non-empty captures newer than your branch base, and without `--saw`.

## 5. Prove and hand off
```bash
bash ops/polaris verify                                        # optional mid-flight check: diff ⊆ files_owned + verify: commands
bash ops/polaris bg run ship-<ID> -- bash ops/polaris handoff --saw "<the after shot vs the bar>"   # gate + landing tail, detached; --saw only when pack asked for captures
...
That tail can WAIT (a busy lease polls up to 10 minutes — deliberately exactly the harness's 600s tool cap), which is why the recipe above detaches it: `bg run ship-<ID>` + chunked `bg wait --max 300` (half the cap), repeating `bg wait` until the rc is not 2 — never a foreground wait that can cross the cap, never a background notification — and why `--saw` rides that line inline: a detached job has nobody at a keyboard to prompt. Parse the rc, not the prose. A `queued:` line (rc 3) means the lane stayed busy, not broken:
