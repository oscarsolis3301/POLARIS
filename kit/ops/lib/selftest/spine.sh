...
selftest() { # end-to-end mechanics drill in a throwaway repo — run once per new machine.
  ...
  local SELFTEST_LABELS='fmlist tcm report metrics brain rules drift hardening qa finish claudemd remote syncrace notify grant upgrade adopt pr-publish express hint brief newcmds park claimguard busyint pushdegrade route bg'
