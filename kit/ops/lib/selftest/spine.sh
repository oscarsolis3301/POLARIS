...
selftest() { # end-to-end mechanics drill in a throwaway repo — run once per new machine.
  ...
  # selfland; surfaces (T-138, test-surfaces.md § 10) is appended last and its gate sits right after rules.
  local SELFTEST_LABELS='fmlist tcm report metrics brain rules drift hardening qa finish claudemd remote syncrace notify grant upgrade adopt pr-publish express hint brief newcmds park claimguard busyint pushdegrade route bg checkoutguard readyoverlap selfland wtreap awake handover autoupdate surfaces'
