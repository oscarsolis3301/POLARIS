...
```
Every path MUST match a `files_owned` pattern (exact · `dir/` prefix · glob). Then, per non-comment line of `ops/RULES.tsv` (TAB-separated `scope · path|ask|content · ERE · message`) — three kinds, not two: no changed path may match a `path` rule's scope; for each `content` rule, `git diff -U0 <base>...HEAD -- <scope>`'s ADDED lines must not match the ERE; and `ask` = the same denial as `path`, lifted only by a human's recorded approval on the task — so for each `ask` rule, a changed path matching the scope is a violation UNLESS the task you claimed carries an `approved:` entry covering it: take the entry's leading whitespace-delimited token as its scope (everything after it is provenance for humans, never parsed), match it against the changed path with the same pattern semantics as `files_owned`, and pass only on a hit. No `approved:` list, no covering entry, or no claimed task at all → deny, exactly as `path` would. Then run every `verify:` command from the task file; all must exit 0. Stray path, rule hit, or red command → fix or hand back. Never proceed past a violation.
Then the stale-tests proof, from `ops/SURFACES.tsv` (TAB-separated `surface · tests · cmd · note`; `#` lines and blank lines ignored; no file or no rows → skip this proof entirely — an unmapped path is silent by design). Per row, over the same changed-path list: **T** = the changed paths matching `tests` (any change counts); **S** = the changed paths matching `surface` and NOT `tests` whose added lines (`git diff -U0 <base>...HEAD -- <path>`, the `+` lines minus the `+++` header) include at least one that is neither blank nor starts with `#` or `//`. The row is violated when S is non-empty and T is empty — a mapped surface changed and its tests did not — UNLESS the task's `approved:` list covers every path in S (the same matching as the `ask` proof above; announce each clearance as `⚠ SURFACES exception used`). Docs-only and comment-only edits leave S empty, so they never violate; a refactor that touched no test violates by design. Fix it by changing the tests, or a human records the exception — `polaris approve <ID> <surface> -m "why"` — never by editing the row: `ops/SURFACES.tsv` is RULES-guarded and written only by `done`.
...
- **map_delta is a SEPARATE base commit, not part of the board_commit:** ONLY when the task's `map_delta` is non-empty, append those lines to `ops/MAP.md` and commit on `<base>` as `docs(map): <ID> <first delta line>`. This and the `surface:` rows below are the ONLY commits any board mutation ever makes on `<base>`;
- **`surface:` rows ride that same base commit:** for each item of the task's `surface:` list write one row `<surface><TAB><tests><TAB><cmd or -><TAB><note> [<ID>]` to `ops/SURFACES.tsv` (grammar: the first token is the surface, `tests:` is required and its value the next token, `cmd:` is everything up to ` note: ` or the end — absent means `-`, `note:` is the rest — absent means the task's `title:`); seed the file with its comment header first when it is absent. Skip, with a `⚠` note and never a die, a malformed item, a `tests` glob that covers its own surface, or a (surface, tests) pair the file already has. Then commit on `<base>`: a `docs(map):` commit made above takes `ops/SURFACES.tsv` too; rows only → `docs(surfaces): <ID> <first row's surface>` over `ops/SURFACES.tsv` alone. No rows → byte-identical to before. The file is RULES-guarded `path` — this recipe is its ONLY writer;
...
What `ops/polaris land --express <ID>` does by hand: one small task — the lane `triage` names — collapses the whole
...
- Board mutations = `board_commit` on `refs/heads/polaris/board` in the primary checkout ONLY (the sole `<base>` commits any mutation makes are `done`'s `docs(map):` / `docs(surfaces):` when `map_delta` or `surface:` is non-empty); code = `feat/<ID>` in worktrees ONLY.
